...
```bash
python -m openclaw_enhance.validation.live_probes recovery-worker --openclaw-home "$OPENCLAW_HOME" --worker oe-tool-recovery
```
...
**stdout:**
```
{"marker": "PROBE_RECOVERY_WORKER_OK", "ok": true, "probe": "recovery-worker", "worker": "oe-tool-recovery"}
```
